...
void cvsync_openssl_update(void *, const void *, size_t);
void cvsync_openssl_final(void *, uint8_t *, unsigned int *);
void cvsync_openssl_destroy(void *);

const struct hash_args MD5_args = {
	cvsync_MD5_init, cvsync_openssl_update, cvsync_MD5_final,
	cvsync_openssl_destroy,
	HASH_SIZE_MD5
};

const struct hash_args SHA1_args = {
	cvsync_SHA1_init, cvsync_openssl_update, cvsync_SHA1_final,
	cvsync_openssl_destroy,
	HASH_SIZE_SHA1
};

const struct hash_args SHA256_args = {
	cvsync_SHA256_init, cvsync_openssl_update, cvsync_SHA256_final,
	cvsync_openssl_destroy,
	HASH_SIZE_SHA256
};
...
cvsync_MD5_final(void *ctx, uint8_t *buffer)
{
	...
	cvsync_openssl_final(ctx, buffer, &s);
}
...
void
cvsync_openssl_update(void *ctx, const void *buffer, size_t bufsize)
{
	EVP_DigestUpdate(ctx, buffer, bufsize);
}

void
cvsync_openssl_final(void *ctx, uint8_t *buffer, unsigned int *s)
{
